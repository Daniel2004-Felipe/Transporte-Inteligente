...
public class MapaActivity extends AppCompatActivity {
    ...
    private ResultadoRuta resultadoRuta = new ResultadoRuta();

    private static final int CODIGO_COARSE = 500;
    private TextView tvdistance_info;
    private TextView Tvtiempo;
    private EditText buscadorDestino;
    private Button buscarParadero;
    private Button detalleRutas;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Vistas
        tvdistance_info = findViewById(R.id.distance_info);
        ...
        detalleRutas= (Button) findViewById(R.id.detalle);
       ...
       detalleRutas.setOnClickListener(new View.OnClickListener() {
           @Override
           public void onClick(View v) {
               detallesIntents();
                ...
                DistanciaTotal(rutas.getParaderos(), p, m, new RutaCallback() {
                    ...
                    public void onSuccess(double distancia, double tiempo, List<GeoPoint> geopoints) {
                        ...
                        if (completadas[0] == totalLlamadas) {
                             //enviar Textos
                            Tvtiempo.setText("Tiempo: " + String.format(Locale.getDefault(), "%.2f min", tiempo));
                            tvdistance_info.setText(
                                    "Distancia: " + String.format(Locale.getDefault(), "%.0f m", distancia)
                            );
    ...
    // ------------------ Manejo Intents (datos) ------------------
    public void detallesIntents(){

        if (resultadoRuta.mejorOrigen == null || resultadoRuta.mejorDestino == null) {
            Toast.makeText(this, "No hay ruta calculada aún", Toast.LENGTH_SHORT).show();
            return;
        }

        Intent intent = new Intent(MapaActivity.this, DetalleRutaActivity.class);

        intent.putExtra("distancia", resultadoRuta.mejorDistancia);
        intent.putExtra("tiempo", 0.0); // si lo tienes calculado
        intent.putExtra("origen", resultadoRuta.mejorOrigen.getNombre()); // si tienes nombre del paradero
        intent.putExtra("destino", resultadoRuta.mejorDestino.getNombre());
